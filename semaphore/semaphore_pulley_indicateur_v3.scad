...
screw_diameter = 4 + 2*loose_slack;
screw_head_embedment = 3.5;
...
motor_shaft = screw_diameter;	// NEMA17 motor shaft exact diameter = 5
...
pulley_t_ht = 6.5;	// length of toothed part of pulley, standard = 12
...
total_ht = pulley_t_ht + pulley_b_ht + retainer * retainer_ht + idler * idler_ht;
...
if ( profile == 12 ) { 
    difference() {
        ...
        // Screw head embedment
        translate([0, 0, total_ht - screw_head_embedment])
            cylinder(d=screw_diameter*7/4, h=screw_head_embedment + 1, $fn=6);
    }
